...
for (i in 11:160) {
  if(i%%2 == 1){
    ...
    data[[i-10]] = fileset[[i]] %>%
      ...
      mutate(., group.no = (i+1)/2, player.no = i, Dprice = StockPrice - lag(StockPrice)) %>%
      select(.,group.no, player.no, Trials, StockPrice, Dprice, Cash, Stock, TotalAsset, Decision, opp_Cash, opp_Stock, opp_TotalAsset, opp_Decision)
  }else{
    p1 = i-1
    data[[i-10]] = fileset[[i]] %>%
      ...
      mutate(., group.no = (i)/2, player.no = i, Dprice = StockPrice - lag(StockPrice)) %>%
      select(.,group.no, player.no, Trials, StockPrice, Dprice, Cash, Stock, TotalAsset, Decision, opp_Cash, opp_Stock, opp_TotalAsset, opp_Decision)
  }
}
#select data
